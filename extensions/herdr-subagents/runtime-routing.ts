...
export function resolveRuntimePlan(
  ...
  let modelSource: RuntimeSource = modelSelection.source;
  let runtimeMismatch: string | undefined;
  let requestedModelForReturn = modelSelection.value;
  ...
  if (modelSelection.value) {
    ...
    if (!found) {
      runtimeMismatch = `preferred model ${JSON.stringify(modelSelection.value)} not found — falling back to default ${parent.provider}/${parent.modelId}`;
      modelSource = "parent";
      requestedModelForReturn = undefined;
      // keep parent provider/modelId/selectedModel
    } else if (!registry.hasConfiguredAuth(found)) {
      runtimeMismatch = `preferred model ${JSON.stringify(modelSelection.value)} has no configured auth — falling back to default ${parent.provider}/${parent.modelId}`;
      modelSource = "parent";
      requestedModelForReturn = undefined;
    } else {
  ...
  return {
    ...
    ...(requestedModelForReturn ? { requestedModel: requestedModelForReturn } : {}),
