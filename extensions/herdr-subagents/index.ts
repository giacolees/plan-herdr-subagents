...
async function launchSubagent(
  ...
  // Forward parent identity so planner can forward todos directly to the main session pane
  if (process.env.HERDR_PANE_ID) {
    envParts.push(`PI_PARENT_PANE_ID=${shellQuote(process.env.HERDR_PANE_ID)}`);
  }
  if (sessionFile) {
    envParts.push(`PI_PARENT_SESSION_FILE=${shellQuote(sessionFile)}`);
  }
  try {
