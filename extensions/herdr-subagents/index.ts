...
function isStubFile(destination: string, template: string): boolean {
  ...
  try {
    ...
    if (
      dest.includes("Seeded (mine)") ||
      dest.includes("Seeded from repo") ||
      dest.includes("Auto-generated by `/prime`")
    )
      return false;
...
function mineRepo(cwd: string): string[] {
  ...
  for (const file of PROJECT_CONTEXT_FILES) {
    ...
    try {
      let content: string;
      if (file === "architecture.md") content = generateArchitecture(cwd);
      else if (file === "conventions.md") content = generateConventions(cwd);
      else if (file === "decisions.md") content = generateDecisions(cwd);
      else content = generateDomain(cwd);
      writeFileSync(destination, content, "utf8");
      created.push(destination);
    } catch { void 0; }
  }
...
function generateArchitecture(cwd: string): string {
  ...
  const topDirs = readdirSync(cwd, { withFileTypes: true })
    ...
    .map((d) => d.name)
    .slice(0, 16);
  const hasExtensions = existsSync(join(cwd, "extensions"));
  const hasAgents = existsSync(join(cwd, "agents"));
  const hasSkills = existsSync(join(cwd, "skills"));
  const hasSrc = existsSync(join(cwd, "src"));
  const hasDocs = existsSync(join(cwd, "docs"));
  const extList = hasExtensions
    ? readdirSync(join(cwd, "extensions"), { withFileTypes: true })
        .filter((d) => d.isDirectory())
        .map((d) => d.name)
        .join(", ")
    : "(none)";
  const agentList = hasAgents
    ? readdirSync(join(cwd, "agents"))
        .filter((f) => f.endsWith(".md"))
        .join(", ")
    : "(none)";
  const deps = pkg?.dependencies ? Object.keys(pkg.dependencies).slice(0, 12).join(", ") : "(none)";
  const desc = pkg?.description ? pkg.description : "No package description.";
  const name = pkg?.name ?? "unknown";
  const typeField = pkg?.type ? `\n- module type: \`${pkg.type}\`` : "";

  return `# Architecture

Auto-generated by \`/prime\` on ${new Date().toISOString().slice(0, 10)} — review and refine. Source: \`package.json\` + directory scan.

${desc}

## Project

- package: \`${name}\`${pkg?.version ? ` @ \`${pkg.version}\`` : ""}${typeField}
- top entries: ${topEntries || "(empty)"}
- dependencies: ${deps}

## Structure

- top dirs: ${topDirs.join(", ") || "(none)"}
${hasSrc ? "- `src/` — source code\n" : ""}${hasExtensions ? `- \`extensions/\` — ${extList}\n` : ""}${hasAgents ? `- \`agents/\` — ${agentList}\n` : ""}${hasSkills ? "- `skills/` — local skills\n" : ""}${hasDocs ? "- `docs/` — documentation\n" : ""}
## Stack hints

- ${existsSync(join(cwd, "tsconfig.json")) ? "TypeScript (tsconfig.json present)" : "No tsconfig.json"}
- ${existsSync(join(cwd, "package.json")) ? "Node / npm project" : "No package.json"}
- ${existsSync(join(cwd, "pyproject.toml")) || existsSync(join(cwd, "requirements.txt")) ? "Python detected" : "No Python markers"}

## Notes

Record only durable, code-backed facts. Replace this auto-generated draft with curated architecture after first \`/plan\` review. Do not add task plans or debugging notes.
`;
}

function generateConventions(cwd: string): string {
  const hasTsconfig = existsSync(join(cwd, "tsconfig.json"));
  const hasEslint = existsSync(join(cwd, ".eslintrc")) || existsSync(join(cwd, ".eslintrc.json")) || existsSync(join(cwd, "eslint.config.js"));
  const hasBiome = existsSync(join(cwd, "biome.json"));
  const hasPrettier = existsSync(join(cwd, ".prettierrc")) || existsSync(join(cwd, ".prettierrc.json"));
  const hasVitest = existsSync(join(cwd, "vitest.config.ts")) || existsSync(join(cwd, "vitest.config.js"));
  const hasJest = existsSync(join(cwd, "jest.config.js"));
  let pkgType = "";
  try {
    const pkg = JSON.parse(readFileSync(join(cwd, "package.json"), "utf8"));
    pkgType = pkg.type === "module" ? "ESM (`type: module`)" : pkg.type ? `type: ${pkg.type}` : "CJS (no type field)";
  } catch { void 0; }
  return `# Conventions

Auto-generated by \`/prime\` on ${new Date().toISOString().slice(0, 10)} — refine via \`/plan\` grilling.

## General

- Minimal, focused changes — look like they belong. No redesign beyond scope.
- Read before editing. Ground in-repo APIs via \`symbol_search\`/\`module_report\`; external via \`resolve-library-id\` + \`query-docs\`.
- Keep \`.agent/\` stable — only permanent knowledge. No debugging sessions or task reasoning.

## Stack observed

- ${pkgType}
- ${hasTsconfig ? "TypeScript (tsconfig.json)" : "No tsconfig.json"}
- lint: ${hasEslint ? "eslint" : hasBiome ? "biome" : hasPrettier ? "prettier" : "(none detected)"}
- test: ${hasVitest ? "vitest" : hasJest ? "jest" : "(none detected) — add acceptance commands per ISC"}

## Planning

- Pointer-based plan: terms → \`CONTEXT.md\`, decisions → \`docs/adr/\`, no duplication.
- Todo bodies ≤5 lines; effort 2-5 min per todo.

## Workers / Reviewer

- Claim todo before implementing; run acceptance + \`lsp_diagnostics\` + \`lens_diagnostics mode=all\` before closing.
- Never edit \`plan.md\`/\`CONTEXT.md\`/ADRs; flag drift via summary.
`;
}

function generateDecisions(cwd: string): string {
  const hasAdr = existsSync(join(cwd, "docs", "adr"));
  const adrList = hasAdr
    ? readdirSync(join(cwd, "docs", "adr"))
        .filter((f) => f.endsWith(".md"))
        .slice(0, 8)
        .join(", ")
    : "";
...
Auto-generated by \`/prime\` on ${new Date().toISOString().slice(0, 10)}.

Permanent architectural decisions and their short rationale. Promote only hard-to-reverse + surprising + traded-off decisions.

${hasAdr ? `- existing ADRs: ${adrList || "(none yet)"}\n- see \`docs/adr/NNNN-slug.md\`\n` : "- no `docs/adr/` yet — create on first hard decision (see `domain-modeling` skill)\n"}
## Seeded

- (no decisions recorded yet — add via knowledge-update after Reviewer)
`;
}

function generateDomain(cwd: string): string {
  const hasGlossary = existsSync(join(cwd, "CONTEXT.md"));
  const glossaryHint = hasGlossary ? "- product glossary: `CONTEXT.md` exists — see it for ubiquitous language\n" : "- no `CONTEXT.md` yet — planner will create glossary lazily via `domain-modeling`\n";
  return `# Domain

Auto-generated by \`/prime\` on ${new Date().toISOString().slice(0, 10)}.

Stable project-specific terms and rules. Use concise definitions and preferred vocabulary.

${glossaryHint}- workflow terms (Plan, ISC, Pointer contract, Grilling, Scout/Worker/Reviewer) — see \`.agent/domain.md\` in this package\n
## Seeded

- (no project terms yet — crystallise via grilling, then update \`CONTEXT.md\` / this file)\n`;
}
  ...
  try {
    ...
    if (running.cli === "claude") {
      ...
      if (running.sentinelFile) {
        ...
        try { unlinkSync(running.sentinelFile); } catch { void 0; }
        try { unlinkSync(running.sentinelFile + ".transcript"); } catch { void 0; }
      }
...
export default function subagentsExtension(pi: ExtensionAPI) {
  ...
  pi.registerCommand("prime", {
    description: "Prime .agent cache: copies from main in a worktree, otherwise mines via primer scout (idempotent, never overwrites)",
    handler: async (_args, ctx) => {
      try {
        if (isGitWorktree(ctx.cwd)) {
          const files = copyFromMain(ctx.cwd);
          if (files.length === 0) {
            ctx.ui.notify(`.agent cache already primed (worktree).`, "info");
          } else {
            const names = files.map((f) => f.replace(ctx.cwd + "/", "")).join(", ");
            ctx.ui.notify(`Primed .agent cache from worktree: ${names}`, "info");
          }
          return;
        }

        // Mine mode: check if population is needed, then spawn primer scout
        const needs = PROJECT_CONTEXT_FILES.filter((file) => {
          const dest = join(ctx.cwd, ".agent", file);
          const tmpl = join(PROJECT_CONTEXT_TEMPLATE_DIR, file);
          return !existsSync(dest) || isStubFile(dest, tmpl);
        });
        if (needs.length === 0) {
          ctx.ui.notify(`.agent cache already primed (mine).`, "info");
          return;
        }

        ctx.ui.notify(`Spawning primer to populate .agent (${needs.join(", ")})...`, "info");
        const task = `Populate .agent cache in ${ctx.cwd} — ${needs.length} file(s) are missing or stubs (${needs.join(", ")}). Read package.json, README, directory structure, extensions/, agents/, skills/ and write full .agent/*.md files (architecture.md, conventions.md, decisions.md, domain.md). Never overwrite a non-stub file (check templates/agent/*.md). Populate each stub with Auto-generated by \`/prime\` (primer) header plus code-backed facts. Create .agent/plans/ dir if missing. Report files created.`;
        pi.sendUserMessage(`Use subagent with agent: "primer", name: "Primer", task: ${JSON.stringify(task)}`);
      } catch (error) {
